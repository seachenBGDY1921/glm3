...
class LocalDocQA:
    ...
    def get_knowledge_based_answer(self, query, vs_path, chat_history=[], streaming: bool = STREAMING):
        ...
        # 按距离由小到大排序
        sorted_related_docs_with_score = sorted(related_docs_with_score, key=lambda x: x.metadata['score'])
        for item in sorted_related_docs_with_score:
            score = item.metadata['score']
            score_percent = round((1000-score)/1000, 2)*100
            item.metadata['score'] = score_percent
        torch_gc()
        if len(sorted_related_docs_with_score) > 0:
            prompt = generate_prompt(sorted_related_docs_with_score, query)
        else:
            prompt = query
        ...
        for answer_result in answer_result_stream_result['answer_result_stream']:
            ...
            response = {"query": query,
                        "result": resp,
                        "source_documents": sorted_related_docs_with_score}
    ...
    def list_file_from_vector_store(self,
        ...
        print(len(vector_store.index_to_docstore_id.keys()))
